...
export default function Updateschedule() {
  ...
  useEffect(() => {
    try {
      const fetchworkout = async () => {
        const res = await fetch(`/api/user/getschedule?itemId=${Id}`);
        ...
        if (res.ok) {
            const selected = data.Items.find((workout) => workout._id === Id);
            if (selected) {
  ...
  return (
    <div className=" ">
         ...
         <h1 className="text-5xl font-serif  whitespace-nowrap ml-80 mt-14 text-slate-900">Update Schedule</h1>
      <div className="flex p-3 max-w-3xl mx-auto flex-col md:flex-row md:items-center gap-5">
       
        
          <div className="w-[550px] h-[850px] border rounded-xl  mt-2 bg-white bg-opacity-25 shadow-xl ml-60 shadow-xl">
